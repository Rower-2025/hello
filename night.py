...
turtle.title("打砖块游戏 - 夜间模式")      # 设置窗口标题
...
# 创建初始球
def create_ball(x=0, y=0, dx=None, dy=None):
    """创建一个新球并添加到球列表中"""
...
try:
    ...
    while not game_over:  # 游戏主循环
        ...
        # 移动所有球
        for ball in balls[:]:  # 使用切片创建副本，避免遍历过程中修改列表
            # 移动球
            ...
            for brick in bricks[:]:  # 使用切片创建副本，避免遍历过程中修改列表
                ...
                if (brick_x - BRICK_WIDTH/2 < ball.xcor() < brick_x + BRICK_WIDTH/2) and \
                   (brick_y - BRICK_HEIGHT/2 < ball.ycor() < brick_y + BRICK_HEIGHT/2):
                    ...
                    # 球碰到砖块后分裂为两个
                    if len(balls) < 5:  # 限制最大球数，避免性能问题
                        # 创建第二个球，方向略有不同
                        angle_offset = random.uniform(-math.pi/6, math.pi/6)  # 随机角度偏移
                        speed = math.sqrt(ball.dx**2 + ball.dy**2)  # 获取当前速度大小
                        new_dx = speed * math.cos(math.atan2(ball.dy, ball.dx) + angle_offset)
                        new_dy = speed * math.sin(math.atan2(ball.dy, ball.dx) + angle_offset)
                        create_ball(ball.xcor(), ball.ycor(), new_dx, new_dy)
               ...
               (ball_y < paddle_y + PADDLE_HEIGHT/2 and ball_y > paddle_y - PADDLE_HEIGHT/2):
                ...
                if len(balls) > 1:
                    ...
                    for other_ball in balls[:]:
                        if other_ball != ball:
                            other_ball.hideturtle()  # 隐藏其他球
                            balls.remove(other_ball)  # 从列表中移除其他球
    ...
    if game_over:
        ...
        instruction_text.write("操作说明:\n长按方向键↑↓移动板子\n红色砖块得1分，黄色砖块得2分\n球碰到砖块后会分裂为两个\n接住一球后另一球会消失", align="center", font=("Courier", 16, "normal"))  # 写入操作说明
